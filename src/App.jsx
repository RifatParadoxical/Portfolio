...
>
  ...
  <div className="flex-1 mt-4 lg:mt-0 text-center lg:text-left">
    <motion.h1
     whileHover={{x: 12}}
     className="p-0 m-0 text-3xl origin-right cursor-pointer sm:text-4xl md:text-5xl lg:text-6xl font-extrabold bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">
      MD RIFAT
    </motion.h1>

    <motion.p
    whileHover={{x: 12}}
    className="mt-3 text-sm sm:text-base md:text-lg text-slate-300 max-w-xl mx-auto lg:mx-0">
      Web scraping & data automation · Clean deliverables (CSV / JSON / Sheets)
    </motion.p>

    <motion.div
    className="mt-5 flex flex-wrap justify-center lg:justify-start gap-3">
      <motion.button
        whileTap={{ scale: 0.98 }}
        whileHover={{scale:1.1}}
        onClick={scrollToProjects}
        className="hover:bg-cyan-500 text-black bg-zinc-300 px-5 py-2.5 rounded-full font-semibold shadow hover:shadow-2xl focus:outline-none focus:ring-2 focus:ring-cyan-400"
  ...
  >
    <motion.div
      ...
      className="rounded-full overflow-hidden border border-slate-700 shadow-2xl bg-conic-180 from-indigo-600 via-indigo-50 to-indigo-600 p-2"
      aria-hidden="false"
    >
      <img 
        src="photo.webp"
        ...
        loading="lazy"
        className="w-[70vw] aspect-square sm:w-[50vw] md:w-[40vw] lg:w-72 h-auto object-cover rounded-full"
      />
    </motion.div>
  </motion.div>
</motion.header>
      ...
      <section className="max-w-6xl mx-auto mt-6 mb-6 px-6">
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.8 }} className="bg-slate-900 p-6 rounded-3xl border border-slate-800 -mt-10 shadow-lg flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <div>
              <motion.div whileHover={{x: 12}} className="text-sm text-slate-400 hidden md:block">Primary</motion.div>
              <motion.div whileHover={{x: 12}} className="font-medium hidden md:block">Python, Selenium, Scrapy, Playwright</motion.div>
            </div>
          </div>
      ...
      <section id="about" className="max-w-5xl mx-auto px-6 py-12">
        <div className="grid md:grid-cols-2 gap-8 items-start">
          <div>
            <motion.h2 whileHover={{x: 12}} className="text-2xl cursor-pointer font-bold mb-3">About Me</motion.h2>
            <motion.p whileHover={{x:12}} className="text-slate-300">I build reliable scrapers and data pipelines that convert messy web data into clean, actionable datasets. I focus on reliable delivery, clear README, and reproducible code.</motion.p>
            <div className="mt-6">
              <motion.h3 whileHover={{x: 12}} className="font-semibold">Education</motion.h3>
              <motion.ul whileHover={{x: 12}} className="mt-2 text-sm text-slate-400">
          ...
          <div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              ...
              <motion.div whileHover={{x:7}} className="p-3 bg-slate-900 rounded-xl border border-slate-800">Volunteer</motion.div>
              <motion.div whileHover={{x:7}} className="p-3 bg-slate-900 rounded-xl border border-slate-800">Science Fest Competitor</motion.div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section ref={projectsRef} id="projects" className="max-w-6xl mx-auto px-6 py-12">
        <motion.h2 whileHover={{x:25}} className="text-3xl font-bold mb-6">Featured Projects</motion.h2>
            ...
            <motion.article key={p.id} layout whileHover={{ scale: 1.01 }} onClick={() => setActive(p)} className="group relative bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow cursor-pointer">
              ...
              <div className="p-4">
                <div className="flex justify-between items-start">
                  <motion.h3 whileHover={{x:12}} className="font-semibold text-lg">{p.title}</motion.h3>
                  <div className="text-xs text-slate-500">{p.updated}</div>
                </div>
                <motion.p whileHover={{x:12}} className="text-slate-400 text-sm mt-2">{p.desc}</motion.p>
          ...
          {active && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
              <motion.div initial={{ y: 20, scale: 0.95 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20, scale: 0.95 }} transition={{ duration: 0.25 }} className="bg-slate-900 w-11/12 md:w-3/4 lg:w-2/3 rounded-2xl p-6 border border-slate-800">
                <div className="flex justify-between items-start">
                  <div>
                    <motion.h3 whileHover={{x:9}} className="text-xl font-bold">{active.title}</motion.h3>
                    <motion.p whileHover={{x:9}} className="text-slate-400 text-sm mt-2">{active.desc}</motion.p>
                    <motion.div whileHover={{x:9}} className="mt-3 text-xs text-slate-500">Updated: {active.updated}</motion.div>
                  </div>

                  <div className="flex gap-3">
                    <a href={active.link} target="_blank" rel="noreferrer" className="px-3 py-2 border hover:bg-white hover:text-black rounded">View Repo</a>
                    <button onClick={() => setActive(null)} className="px-2 py-2 border hover:bg-red-600 rounded">Close</button>
                  </div>
                </div>

                <div className="mt-4 grid md:grid-cols-2 gap-4">
                  <img src={active.image} alt="preview" className="w-full h-56 object-cover rounded" />
                  <motion.div whileHover={{x:10}} className="text-sm text-slate-300">
      ...
      <section className="max-w-5xl mx-auto px-6 py-12">
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            <motion.h3 whileHover={{x: 12}} className="text-2xl font-bold mb-4">Client Feedback</motion.h3>
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow">
              <motion.p whileHover={{x: 10}} className="text-slate-300 italic">“Delivered a scraper that saved our team 10+ hours weekly. Clear code, fast delivery.”</motion.p>
              <motion.div whileHover={{x: 10}} className="mt-4 text-sm text-cyan-400">— E-commerce Analyst, Remote</motion.div>
            </motion.div>
          </div>

          <div>
            <motion.h3 whileHover={{x:12}} className="text-2xl font-bold mb-4">Contact</motion.h3>
            <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800">
              <div className="flex flex-col gap-3 text-sm">
                <motion.div whileHover={{x:9}} className="flex items-center gap-3"><Mail size={18}/> <a href="mailto:zikkiz275@gmail.com" className="underline">zikkiz275@gmail.com</a></motion.div>
                <motion.div whileHover={{x:9}} className="flex items-center gap-3">📞 <a href="tel:+8801944553174" className="underline">+880-194-455-3174</a></motion.div>
                <motion.div whileHover={{x:9}} className="flex items-center gap-3"><Github size={18}/> <a href="https://github.com/RifatParadoxical" className="underline">github.com/RifatParadoxical</a></motion.div>
                <motion.div whileHover={{x:9}} className="flex items-center gap-3"><Linkedin size={18}/> <a href="https://linkedin.com/in/md-rifat-dev" className="underline">linkedin.com/in/md-rifat-dev</a></motion.div>
                </div>
      ...
      <motion.footer whileHover={{scaleY:1.2}} className="border-t border-slate-800 py-8 text-center text-slate-500 text-sm">
